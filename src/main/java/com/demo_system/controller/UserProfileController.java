...
public class UserProfileController {
     ...
     */
    @PostMapping("/avatar")
    public Response uploadAvatar(@RequestAttribute("userId") Long userId,
                                 @RequestParam("file") MultipartFile file) {
        if (file.isEmpty()) return Response.fail("文件为空");
        try {
            // 确保目录存在
            File folder = new File(WebConfig.UPLOAD_PATH);
            ...
            return Response.ok("上传成功", avatarUrl);
        } catch (Exception e) {
